...
export default function SearchFegrn ({ cadastrData , setCadastrData }) {
  ...
  const sendDataToServer = async (fullAddress, number) => {
    ...
    if (checker) {
      router.push(`/reestr/${formattedInput}`)
    }
          ...
          <form id={style.request_form}>
            ...
            <div className={style.formData}>
              ...
              <div className={style.formDataText}>
                <p>
                  Единая форма поиска недвижимости для заказа отчетов. Позволяет произвести поиск по кадастровому номеру или адресу объекта.
                    ...
                    <div className={`${style["form-submit1"]} ${style.table} ${style["step-1"]}`}>
                      <div className={style.faq1__cont}>
                        <div className={style.faq__title}>
                          <ul>
                            {it.type === 'PARCEL' ? <li className={style.type}>Земельный участок</li> : (
                              it.type === 'OKS' ? <li className={style.type}>Здание</li> : (
                                it.type === "FLAT" ? <li className={style.type}>Квартира</li> : <li className={style.type}>Объект</li>
                              )
                            )}
      ...
      {tooltipsError && (
        <div className={style.dataResult1}>
          <p className={style.dataError}>
            В настоящее время наблюдаем проблемы с поиском по адресу. Попробуйте произвести поиск еще раз, либо используйте для поиска кадастровый номер, например
            <span
              aria-hidden="true"
