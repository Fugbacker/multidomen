...
export default function SearchFcad ({ cadastrData , setCadastrData }) {
  const tokenList = [
  ...
  if (enterText === 'г Саратов, ул Ульяновская, д 32, кв 55') {
    fullAddress = 'г Саратов, ул Ульяновская, д 32, кв 55'
  }
    ...
    <>
             ...
             <form id={style.request_form}>
              <div className={`${style["tab-cont"]} ${style.active}`}>
                <div className={style["form-row"]}>
                  ...
                  <div className={`${style.colm} ${style.colm1} ${style.dadata_box}`}>
                    ...
                    {loading ? (
                      <div className={style.pulseLoader4}>
                        <PulseLoader color="#0073B6" size={10} />
                      </div>
                    ) : ('')}
                  ...
                  <div className={style.search__example}>
                    Пример:
                    <span
                      aria-hidden="true"
                      className={`${style.a} ${style["_blueback"]} ${style["_inner"]} ${style.js__searchExample}`}
                      onClick={() => {
                        setEnterText('г Саратов, ул Ульяновская, д 32, кв 55');
                        setValidInput(true);
                        setCadastrData('');
                      }}
                    >
                      Саратов, ул Ульяновская, д 32, кв 55
                    </span>
                    {' '}или{' '}
                    <span
                      aria-hidden="true"
      ...
      {cadastrData.length > 1 && (
        <div className={style.stepform1}>
          <div className={style.searchObjects} ref={ref}>
            Количество найденных объектов {cadastrData.length}шт.
          </div>
                    ...
                    <div className={style.faq__title}>
                      <ul>
                        {it.type === 'PARCEL' ? (
                          ...
                          <li className={style.type}>Здание</li>
                        ) : it.type === "FLAT" ? (
                          <li className={style.type}>Квартира</li>
                        ) : (
                          <li className={style.type}>Объект</li>
                        )}
                        <li className={style.adr}>{it?.full_name || it?.addressNotes}</li>
                        <li className={style.cad_n}>{it?.cadnum || it?.objectCn}</li>
                        <li className={style.choose}>
                          <p
                            className={style.go}
                            onClick={() => {
                              setLoader({ ...loader, [index]: true });
                              sendDataToServer(fullAddress, it.cadnum || it?.objectCn);
                              setValidInput(true);
                            }}
                          >
                            {loader[index] ? (
      ...
      {tooltipsError && (
        <div className={style.dataResult1}>
          <p className={style.dataError}>
            В настоящее время наблюдаем проблемы с поиском по адресу. Попробуйте произвести поиск чуть позже, либо используйте для поиска кадастровый номер, например
            <span
              aria-hidden="true"
