...
export default function SearchRosegrn ({ cadastrData , setCadastrData }) {
  const tokenList = [
  ...
  if (enterText === 'г Воронеж, ул Советская, д 53А, кв 33') {
    fullAddress = 'г Воронеж, ул Советская, д 53А, кв 33'
  }
  ...
  return (
    <>
      <div className="tab" id="t1">
        <div className="tabs">
          <div className="stepform_start">
            <form id="request_form">
              <div className="section tab-cont active">
                <div className="form-row">
                  <div className="search__example">
                    ...
                    <span
                      aria-hidden="true"
                      onClick={() => {
                        setEnterText('г Воронеж, ул Советская, д 53А, кв 33');
                        setValidInput(true);
                      }}
                    >
                      Воронеж, ул Советская, д 53А, кв 33
                    </span>{' '}
                    или{' '}
                    <span
                      aria-hidden="true"
                      onClick={() => {
                        setEnterText('36:34:0604026:171');
                        setValidInput(true);
                      }}
                    >
                      36:34:0604026:171
                  ...
                  <div className="colm colm1">
                    ...
                    />
                    {loading && (
                      <div className="pulseLoader4">
                        <DotLoader color="#0073B6" size={20} />
                      </div>
                    )}
                    <div aria-hidden="true" className="searchIcon">
                      {value.length === 0 ? (
                        ...
                        <CloseIcon onClick={clearEnterText} />
                      )}
                    </div>
                  </div>
                  {value.length !== 0 && (
                    <div className="dataResult">
                      {value.slice(0, 5).map((it, index) => {
                        ...
                        return (
                          <div
                            className={`dataItem ${isSelected ? 'selected' : ''}`}
                            key={index}
                            aria-hidden="true"
                            onClick={() => {
                  ...
                  <div className="search__title">
                    Поиск по адресу или кадастровому номеру производится по общедоступным открытым базам данных.
                  </div>
                </div>
              </div>
              <div className="formData">
                <div className="formDataText">
                  {/* <p>
                    Единая онлайн форма поиска недвижимости для заказа отчетов. Произведите поиск по кадастровому номеру
                    или адресу.
                  </p> */}
                </div>
                <button
      ...
      {cadastrData.length > 1 && (
        <div className="stepform1">
          <div className="searchObjects">Найдено объектов: {cadastrData.length} шт.</div>
          <div className="table-row t-header">Выберите объект недвижимости для заказа отчета</div>
          {cadastrData.map((it, index) => (
            <div className="tab" key={index}>
              <div className="form-submit table step-1">
                <div className="faq1__cont">
                  <div className="faq__title">
                     <ul>
                      {(it.type === 'PARCEL' || it?.properties?.options?.land_record_type) ? <li className="type">Земельный участок</li> : (
                        it.type === 'OKS' ? <li className="type">Здание</li> : (
                          it.type === "FLAT" ? <li className="type">Квартира</li> : (
                            it?.properties?.options?.type ? <li className="type">{it?.properties?.options?.type}</li> : <li className="type">Объект</li>
                          )
                        )
                      )}
                      {/* <li className="type">Адрес объекта</li> */}
                      <li className="adr">{it?.full_name || it?.addressNotes || it?.properties?.options?.readable_address}</li>
                      <li className="cad_n">{it?.cadnum || it?.objectCn || it?.properties?.options?.cad_number || it?.properties?.options?.cad_num}</li>
                      <li className="choose">
                        <p
                          className="go"
                          onClick={() => {
      ...
      {error && (
        <div className="dataResult1">
          <div className="dataError">В настоящее время сервер не доступен. Попробуйте произвести поиск чуть позже.</div>
        </div>
      )}
      {tooltipsError && (
        <div className="dataResult1">
          <p className="dataError">
            Объект не найден, или адрес указан некорректно. Попробуйте произвести поиск еще раз, либо используйте для
            поиска кадастровый номер, например{' '}
            <span
              aria-hidden="true"
      ...
      {cadastrData.length === 0 && !check && (
        <div className="dataResult1">
          <div className="dataError">
            В базах Росреестра ничего не найдено. Возможно вы ввели неверный кадастровый номер или не выбрали адрес из
            предложенных подсказок.
          </div>
        </div>
      )}
    </>
