...
export default function SearchMapFegrn ({ cadastrData, setCadastrData, setCadastrNumber, closeChecker, setAlarmMessage, setBaloonData, referer, error, setError }) {
  const tokenList = [
          ...
          <div className={style.stepform_start} id="FormStep1">
            <form id={style.request_form}>
              ...
              <div className={`${style["form-row"]}`}>
                {/* <div className="search__title"><span>Для поиска</span> введите адрес или кадастровый номер объекта:</div> */}
              ...
              <div className={style.formData}>
                <div className={style.search__example}>Пример:
                  <span
                    ...
                    // }}
                  > улица Рябиновая, земельный участок 41А
                  </span>  или
                  <span
                    aria-hidden="true"
                    onClick={() => {
                  ...
                  <div className={style.formDataText}>
                    <p>
                      Поиск земельных участков и зданий на кадастровой карте. Произведите поиск по кадастровому номеру или адресу, либо кликните по интересующему объекту. Сведения взяты из открытых источников НСПД и Росреестра и актуальны на момент запроса.
                    </p>
                  </div>
              </div>
            </form>
      ...
      {cadastrData && cadastrData?.length !== 0 && (
        <div className={style.stepform1}>
          ...
          {cadastrData.map((it, index) => {
            const uniqueKey = +new Date()
            // const cadNum = it?.properties?.options?.cad_num || it?.properties?.options?.cad_number || it?.attrs?.cn
