...
export async function getRosegrnMapProps(context) {
...
async function getDemographyStats(regionName, options = {}, settlementName, macroRegionName) {
  ...
  if (regionData.includes('-')) {
    const regionNumber = regionData.split('-')[1];
    const regionId = parseInt(regionData.split('-')[0]);
  ...
  if (regionData.includes('_')) {
    const regionNumber = regionData.split('_')[1];
    const regionId = parseInt(regionData.split('_')[0]);
    const collection = db.collection('Reestr_geo');
    const array = await collection.find({ 'regionId': regionId }).toArray();
    const settlementsArray = array.map((it) => {
      return {
  ...
  const regionNumber = regionData.split('|')[1];
  const regionCadastrCode = parseInt(regionData.split('|')[0]);
